...
{
    public partial class ucHangSX : UserControl
    {
        ...
        private void FormatGrid()
        {
            for (int i = 0; i < c1FlexGrid1.Cols.Count; i++)
            {
                ...
                { c1FlexGrid1[0, i] = "Hãng sản xuất(*)"; c1FlexGrid1.Cols[i].Visible = true; }
        ...
        private void Delete()
        {
            ...
            if (n >= 1)
            {
                ...
                if (bien == DialogResult.Yes)
                {
                    ...
                    if (loi.Equals("") == true)
                    {
                        try { _ctrlog.Append(Data.use, "Xóa hãng sản xuất: " + sten.Trim()); }
                        catch { }
                        HienThiDS();
                    }
                    else
