...
{
    public partial class AddHoaDonBan : Form
    {
                    ...
                    if (lstmathang != null)
                    {
                        if (objhoadon != null && lstmathang.Count > 0)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    txtID.Text = objhoadon.id.Trim();
                                    _ctrlog.Append(Data.use, "Thêm mới hóa đơn cho khách hàng: " + cbKhachHang.Text.Trim() + " xuất ngày: " + txtNgayXuat.Text.Trim()
                                        + "; id: " + objhoadon.id
                                        + ";\nChi tiết: Tổng tiền hàng: " + txtTongTien.Text + " - Nợ cũ: " + txtNoToaTruoc.Text
                                        + " - Tiền thanh toán: " + txtTienThanhToan.Text + " - Tổng nợ mới: " + txtConNo.Text);
                                }
        ...
        private void Del()
        {
            ...
            if (MessageBox.Show("Xác nhận xóa dữ liệu?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                if (kq.Trim().Equals("") == true)
                {
                    ...
                    //_ctrlog.Append(Data.use, "Xóa hóa đơn của khách hàng: " + cbKhachHang.Text.Trim() + " ; idhd: " + txtID.Text.Trim());
        ...
        private void AddHoaDonBan_Load(object sender, EventArgs e)
        {
        ...
        private void cbKhachHang_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                "dd/MM/yyyy HH:mm:ss",
                CultureInfo.InvariantCulture);
                txtNoToaTruoc.Text = ctrhdb.GetTienConNo(sidkh, dtngaytao).ToString("N0", CultureInfo.InvariantCulture);
